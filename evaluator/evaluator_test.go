...
func TestBuiltinFunctions(t *testing.T) {
	tests := []struct {
		...
		{"rest([1, 2, 3])[0]", 2},
		{"rest([1, 2, 3])[1]", 3},
		{"len(rest([10, 20, 30, 40]))", 3},
		{"rest([10, 20, 30, 40])[0]", 20},
		{"rest([10, 20, 30, 40])[1]", 30},
		{"rest([10, 20, 30, 40])[2]", 40},
		{"rest([1], [2])", "wrong number of arguments. got=2, want=1"},
		{"rest(1)", "argument to `rest` must be ARRAY, got INTEGER"},
		{"rest([])", NULL},
		{"push([], 1)[0]", 1},
		{"len(push([], 1))", 1},
		{"let a = [1, 2]; let b = push(a, 3); len(a)", 2},
		{"let a = [1, 2]; let b = push(a, 3); len(b)", 3},
		{"let a = [1, 2]; let b = push(a, 3); b[0]", 1},
		{"let a = [1, 2]; let b = push(a, 3); b[1]", 2},
		{"let a = [1, 2]; let b = push(a, 3); b[2]", 3},
	}
